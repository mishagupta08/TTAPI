    ...
    public class HomeRepository
    {
        ...
        public async Task<Response> ManageRetailer(R_RetailerMaster retailerDetail, string operation)
        {
            ...
            try
            {
                ...
                if (operation == "Add")
                {
                    using (var db = new TTLimitedEntities())
                    {
                        var user = db.R_RetailerMaster.FirstOrDefault(g => g.Mobile == retailerDetail.Mobile);
                        if (user == null)
                        {
                            ...
                            else
                            {
                                ...
                                //else
                                {
                                    ...
                                    if (String.IsNullOrEmpty(retailerDetail.IsBlock))
                                    {
                                        retailerDetail.IsBlock = "N";
                                    }
            ...
            else
            {
                ...
                if (detail.OrderStatus == Status.Rejected.ToString())
                {
                    ...
                    try
                    {
                        ...
                        points.EarnSpentDate = DateTime.Parse(currentDate, CultureInfo.InvariantCulture);
        ...
        {
            Response responseDetail = new Response();
            R_LogMaster log = new R_LogMaster();
            log.RetailerId = barcodeDetail.RetailerId;
            log.Barcode = barcodeDetail.Barcode;
            ...
            try
            {
                ...
                else if (retailer.IsBlock == "Y")
                {
                    responseDetail.ResponseValue = "Your account has be blocked.";
                }
                else
                {
                    ...
                    /***Un-comment this code for distance*****/
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                log.Result = responseDetail.ResponseValue;

                                entity.R_LogMaster.Add(log);
            ...
            {                
                log.Result = ex.InnerException.InnerException.Message.Substring(0,498);
                entity.R_LogMaster.Add(log);
                await entity.SaveChangesAsync();
            }
